...
import { mockStudents, mockTeachers, mockClasses, mockClassSessions } from '../data/mockData';
import { Student, ClassSession, UserRole } from '../types';
...
const Dashboard: React.FC = () => {
    ...
    const studentsToShow = useMemo(() => {
        if (isAdmin) return mockStudents;
        if (isClassLead && userClassId) return mockStudents.filter(s => s.classId === userClassId);
        return [];
    }, [isAdmin, isClassLead, userClassId]);

    const sessionsToShow = useMemo(() => {
        if (isAdmin) return mockClassSessions;
        if (isClassLead && userClassId) return mockClassSessions.filter(s => s.classId === userClassId);
        return [];
    }, [isAdmin, isClassLead, userClassId]);

    const totalStudents = studentsToShow.filter(s => s.status === 'active').length;
    const totalTeachers = isAdmin ? mockTeachers.filter(t => t.status === 'active').length : 'N/A';
    const totalClasses = isAdmin ? mockClasses.length : (userClassId ? 1 : 0);
    ...
    const classesMap = useMemo(() => new Map(mockClasses.map(cls => [cls.id, cls.className])), []);
    ...
    const statCards = [
        ...
        },
        {
            ...
            value: isAdmin ? totalTeachers : mockTeachers.length,
